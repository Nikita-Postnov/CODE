...
PASSWORDS_DIR = ROOT_DIR / "Passwords"
NOTES_ORDER_PATH = DATA_DIR / "notes_order.json"

keyboard = None
_keyboard_spec = importlib.util.find_spec("keyboard")
if _keyboard_spec is not None and getattr(_keyboard_spec, "loader", None) is not None:
    try:
...
if getattr(sys, "frozen", False):
    APPDIR_PATH = Path(sys.executable).resolve().parent
    try:
        os.chdir(str(APPDIR_PATH))
    except Exception:
        pass
else:
    APPDIR_PATH = Path(__file__).resolve().parent

APPDIR = str(APPDIR_PATH)
DATA_DIR_PATH = APPDIR_PATH / "Data"
NOTES_DIR_PATH = APPDIR_PATH / "Notes"
PASSWORDS_DIR_PATH = APPDIR_PATH / "Passwords"

DATA_DIR = str(DATA_DIR_PATH)
STANDBY_CLEANER_PATH = str(DATA_DIR_PATH / "rammap.exe")
NOTES_DIR = str(NOTES_DIR_PATH)
PASSWORDS_DIR = str(PASSWORDS_DIR_PATH)
SALT_PATH = str(DATA_DIR_PATH / "salt.bin")
SETTINGS_PATH = str(DATA_DIR_PATH / "settings.ini")
ICON_PATH = str(DATA_DIR_PATH / "icon.ico")
TRAY_ICON_PATH = str(DATA_DIR_PATH / "tray_icon.ico")
FILE_ICON_PATH = str(DATA_DIR_PATH / "file.ico")
USER_DICT_PATH = str(DATA_DIR_PATH / "user_dictionary.txt")
NOTES_WIDGET_PATH = str(DATA_DIR_PATH / "desktop_notes.json")
NOTES_WIDGET_BAK = str(DATA_DIR_PATH / "desktop_notes.json.bak")

os.makedirs(DATA_DIR, exist_ok=True)
os.makedirs(NOTES_DIR, exist_ok=True)
os.makedirs(PASSWORDS_DIR, exist_ok=True)
if not os.path.exists(USER_DICT_PATH):
    open(USER_DICT_PATH, "a", encoding="utf-8").close()
    ...
    def __init__(self, parent):
        ...
        try:
            ...
            try:
                ...
                self.spell_checker_ru = _SpellChecker()
            try:
...
class NotesApp(QMainWindow):
    ...
    def _apply_preloaded_notes(self, notes: list[Note]) -> None:
        ...
        if self.notes:
            ...
            last_uuid = self.settings.value("lastNoteUUID", "") or self.settings.value(
                "lastNoteUuid", ""
            )
            if last_uuid:
                for i in range(self.notes_list.count()):
    ...
    def _write_note_cache(self, note: Note, file_path: str | None = None, note_dict: dict | None = None) -> None:
        try:
            ...
            if "content_txt" not in note_dict or not isinstance(note_dict.get("content_txt"), str):
                try:
                    if self.current_note and getattr(self.current_note, "uuid", None) == getattr(note, "uuid", None):
                        doc = QTextDocument()
                        doc.setHtml(self.text_edit.toHtml())
                        note_dict["content_txt"] = doc.toPlainText()
                    else:
            ...
            try:
                if file_path and os.path.exists(file_path):
                    ...
                    p = os.path.join(note_dir, "note.json")
                    if os.path.exists(p):
                        mtime = os.path.getmtime(p)
            except Exception:
                mtime = None
            payload = {"note": note_dict, "note_mtime": mtime}
            cache_path = self._note_cache_path(note.uuid)
            self._atomic_write_cache(cache_path, payload)
        except Exception:
            pass
    ...
    def select_note(self, note: Note | None) -> None:
        ...
        try:
            if previous_note:
                ...
                def _bg_save(snapshot: dict, title: str, uuid_: str, ts: float) -> None:
                    with lock:
    ...
    def show_note_with_attachments(self, note: "Note | None") -> None:
        ...
        if hasattr(self, "attachments_layout"):
            ...
            if not attachments:
                ...
                try:
                    if note_dir and os.path.isdir(note_dir) and note_dir not in self.attachments_watcher.directories():
                    ...
                    while state["i"] < len(attachments) and added < batch:
                        ...
                        if filename.lower().endswith(tuple(IMAGE_EXTENSIONS)):
                            ...
                            if os.path.exists(FILE_ICON_PATH):
                                icon_label.setPixmap(
                                    QPixmap(FILE_ICON_PATH).scaled(48, 48, Qt.KeepAspectRatio, Qt.SmoothTransformation)
                                )
                            else:
                                icon_label.setPixmap(self.style().standardIcon(QStyle.SP_FileIcon).pixmap(32, 32))
                            layout.addWidget(icon_label)
