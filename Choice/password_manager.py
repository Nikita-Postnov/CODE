...
try:
    import pystray
    from PIL import Image, ImageGrab, ImageFilter, ImageTk, ImageEnhance
except Exception:
    pystray = None
    Image = None
    ImageGrab = None
...
PASSWORDS_DIR: str = ""
LAST_PATHS_FILE = DATA_DIR / "password_manager_last_paths.json"


def _load_last_paths() -> tuple[str, str, str]:
    try:
        if not LAST_PATHS_FILE.is_file():
            return "", "", ""
        data = json.loads(LAST_PATHS_FILE.read_text(encoding="utf-8"))
        config_path = str(data.get("config", "") or "")
        salt_path = str(data.get("salt", "") or "")
        pwfile_path = str(data.get("passwords", "") or "")
        return config_path, salt_path, pwfile_path
    except Exception:
        return "", "", ""


def _save_last_paths(config_path: str, salt_path: str, pwfile_path: str) -> None:
    try:
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        payload = {"config": config_path, "salt": salt_path, "passwords": pwfile_path}
        LAST_PATHS_FILE.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    except Exception:
        pass
...
def _pick_file_if_missing(path: str, title: str, filetypes):
    if path and os.path.isfile(path):
        return path
    return tk_filedialog.askopenfilename(title=title, filetypes=filetypes) or ""

def _ensure_paths(config_path: str, salt_path: str, pwfile_path: str) -> None:
    global CONFIG_PATH, SALT_PATH, PASSWORDS_FILE_PATH, PASSWORDS_DIR

    saved_config, saved_salt, saved_pwfile = _load_last_paths()

    if not config_path:
        config_path = saved_config
    if not salt_path:
        salt_path = saved_salt
    if not pwfile_path:
        pwfile_path = saved_pwfile

    tmp_root = None
    try:
        if not (config_path and os.path.isfile(config_path)) or not (salt_path and os.path.isfile(salt_path)) or not (pwfile_path and os.path.isfile(pwfile_path)):
            tmp_root = tk.Tk()
            tmp_root.withdraw()
            tmp_root.attributes("-topmost", True)
            tmp_root.lift()
        config_path = _pick_file_if_missing(
            config_path,
            "Выбери файл конфигурации (config.json)",
            [("JSON", "*.json"), ("All", "*.*")],
        )
        if not config_path:
            raise SystemExit("Не выбран config.json")
        salt_path = _pick_file_if_missing(
            salt_path,
            "Выбери файл соли (salt.bin)",
            [("BIN", "*.bin"), ("All", "*.*")],
        )
        if not salt_path:
            raise SystemExit("Не выбран salt.bin")
        pwfile_path = _pick_file_if_missing(
            pwfile_path,
            "Выбери файл с паролями (passwords.json)",
            [("JSON", "*.json"), ("All", "*.*")],
        )
        if not pwfile_path:
            raise SystemExit("Не выбран passwords.json")
    ...
    PASSWORDS_FILE_PATH = os.path.abspath(pwfile_path)
    PASSWORDS_DIR = os.path.dirname(CONFIG_PATH) or os.path.dirname(PASSWORDS_FILE_PATH)
    if not PASSWORDS_DIR:
        PASSWORDS_DIR = os.getcwd()
    os.makedirs(PASSWORDS_DIR, exist_ok=True)
    _save_last_paths(CONFIG_PATH, SALT_PATH, PASSWORDS_FILE_PATH)
...
class PasswordManager:
    def __init__(self, master_password=None):
        ...
        if PASSWORDS_FILE_PATH:
            self.filename = PASSWORDS_FILE_PATH
...
class AuthenticationDialog(tk.Toplevel):
    def __init__(
        self,
        parent,
        title: str = "Аутентификация",
        center_on: tk.Misc | None = None,
        use_transient: bool = True,
    ):
        super().__init__(parent)
        self._center_on = center_on or parent
        self.title(title)
        ...
        self.after(0, lambda: self.winfo_exists() and self.focus_set())

    def _center_window(self) -> None:
        base = getattr(self, "_center_on", None) or self.parent
    ...
    def _validate_master_password(self, password):
        try:
            passwords_file = PASSWORDS_FILE_PATH
...
class PasswordGeneratorApp:
    def __init__(self, master):
        ...
        self._lock_dialog = None
        self._backup_job = None
        self._locked = False
        self._blur_update_job = None
        self._closing = False
        self._tray_icon = None
        self._lock_active = False
        self._lock_bind_ids: list[tuple[tk.Misc, str, str]] = []
    ...
    def _ensure_tray(self) -> None:
        ...
        icon_path = app_base_dir() / "key.ico"
        ...
        menu = pystray.Menu(
            pystray.MenuItem(
                "Открыть",
                lambda icon, item: self.master.after(0, self._tray_show),
                default=True,
            ),
            pystray.MenuItem(
                "Заблокировать",
                lambda icon, item: self.master.after(0, self.lock_application),
            ),
            pystray.MenuItem(
                "Выход",
                lambda icon, item: self.master.after(0, self._tray_exit),
            ),
        )
    ...
    def _present_lock_ui(self) -> None:
        if not getattr(self, "_lock_active", False):
            return

        ov = getattr(self, "_blur_overlay", None)
        if ov is not None:
            try:
                if ov.winfo_exists():
                    try:
        ...
        dlg = getattr(self, "_lock_dialog", None)
        if dlg is None:
            return
        ...
        try:
            ...
            dlg.after(100, lambda: dlg.winfo_exists() and dlg.attributes("-topmost", False))
        except Exception:
            pass
    ...
    def _tray_show(self) -> None:
        ...
        if getattr(self, "_locked", False):
            if getattr(self, "_lock_active", False) and getattr(self, "_lock_dialog", None) is not None:
                self.master.after(80, self._present_lock_ui)
            else:
                self.master.after(0, self.lock_application)

    def _enable_main_blur(self) -> None:
        self._blur_mode = "none"
        self._blur_error = ""
        ...
        try:
            ...
            self._show_blur_overlay(blur_radius=30, dim_factor=0.60)
            if getattr(self, "_blur_overlay", None) is not None:
                self._blur_mode = "overlay"
                return

            self._blur_error = "Overlay blur: overlay не создан (причина неизвестна)"
        except Exception as exc:
            self._blur_error = f"Overlay blur: исключение: {exc}"


    def _bind_lock_guard(self, widget: tk.Misc, sequence: str, func) -> None:
        bid = widget.bind(sequence, func, add=True)
        self._lock_bind_ids.append((widget, sequence, bid))

    def _request_blur_update(self) -> None:
        if not getattr(self, "_lock_active", False):
            return
        if getattr(self, "_blur_update_job", None) is not None:
            return
        self._blur_update_job = self.master.after(40, self._apply_blur_update)

    def _apply_blur_update(self) -> None:
        self._blur_update_job = None
        if not getattr(self, "_lock_active", False):
            return
        if not self.master.winfo_exists():
        ...
        if getattr(self, "_blur_mode", "none") == "overlay":
            ov = getattr(self, "_blur_overlay", None)
            if ov is None or not ov.winfo_exists():
                self._show_blur_overlay(blur_radius=22, dim_factor=0.72)
            self._update_blur_overlay_geometry()
        elif getattr(self, "_blur_mode", "none") == "dwm":
            try:
                hwnd = int(self.master.winfo_id())
                _set_dwm_blur(hwnd, True)
            except Exception:
                pass
        ...
        if dlg is not None and getattr(dlg, "winfo_exists", lambda: False)():
            try:
                if ov is not None and ov.winfo_exists() and dlg.state() not in ("withdrawn", "iconic"):
                    dlg.lift(ov)
            except Exception:
                pass
    ...
    def _enter_lock_mode(self) -> None:
        ...
        self._enable_main_blur()

        def keep_blur(_event=None):
            self._request_blur_update()

        self._bind_lock_guard(self.master, "<Configure>", keep_blur)
        self._bind_lock_guard(self.master, "<Map>", keep_blur)
        self._bind_lock_guard(self.master, "<FocusIn>", keep_blur)
        self._bind_lock_guard(self.master, "<Visibility>", keep_blur)
        self._bind_lock_guard(self.master, "<Unmap>", keep_blur)
        self._bind_lock_guard(self.master, "<Visibility>", keep_blur)
        self.master.after(0, self._request_blur_update)
    ...
    def _exit_lock_mode(self) -> None:
        ...
        for w, seq, bid in getattr(self, "_lock_bind_ids", []):
            try:
                w.unbind(seq, bid)
            except Exception:
                pass
        self._lock_bind_ids.clear()

        self._disable_main_blur()

    def _update_blur_overlay_geometry(self) -> None:
        ov = getattr(self, "_blur_overlay", None)
        if ov is None or not ov.winfo_exists() or not self.master.winfo_exists():
            return
        try:
        ...
        try:
            ...
            if dlg is not None:
                try:
                    if dlg.winfo_exists() and dlg.state() not in ("withdrawn", "iconic"):
                        dlg.lift(ov)
                except Exception:
                    pass
        except Exception:
    ...
    def _disable_main_blur(self) -> None:
        try:
            job = getattr(self, "_blur_update_job", None)
            if job is not None:
                self._blur_update_job = None
                try:
        ...
        mode = getattr(self, "_blur_mode", "none")
        self._blur_mode = "none"
        self._blur_error = ""
    ...
    def _show_blur_overlay(self, blur_radius: int = 14, dim_factor: float = 0.78) -> None:
        if ImageGrab is None or ImageFilter is None or ImageTk is None or ImageEnhance is None:
            return
        if getattr(self, "_blur_overlay", None) is not None:
            return
        if not self.master.winfo_exists():
        ...
        try:
            ...
            ov = tk.Toplevel(self.master)
            ov.protocol("WM_DELETE_WINDOW", lambda: None)
            ov.bind("<Alt-F4>", lambda e: "break")
            ov.bind("<Escape>", lambda e: "break")
            ...
            ov.after(0, lambda: ov.attributes("-topmost", False))
            tk.Frame(ov, background="black").pack(fill="both", expand=True)
            self._blur_overlay = ov
            return
        ov = tk.Toplevel(self.master)
        ov.protocol("WM_DELETE_WINDOW", lambda: None)
        ov.bind("<Alt-F4>", lambda e: "break")
        ...
        lbl = tk.Label(ov, image=photo, borderwidth=0, highlightthickness=0)
        lbl.image = photo
        lbl.pack(fill="both", expand=True)
        self._blur_overlay = ov

    def _hide_blur_overlay(self) -> None:
        try:
            if self.master.winfo_exists():
                try:
                    self.master.attributes("-disabled", False)
    ...
    def _on_close(self):
        ...
        if self._tray_icon is None:
            if messagebox.askyesno("Выход", "Трей недоступен. Закрыть приложение полностью?"):
                self._closing = True
                if self.master.winfo_exists():
                    self.master.destroy()
            return
    ...
    def lock_application(self):
        ...
        if getattr(self, "_lock_active", False):
            return

        self._enter_lock_mode()

        try:
            center_on = getattr(self, "_blur_overlay", None)
            if center_on is None or not getattr(center_on, "winfo_exists", lambda: False)():
            ...
            self._lock_dialog = auth_dialog

            def _hide_lock_windows() -> None:
                ov = getattr(self, "_blur_overlay", None)
                if ov is not None:
                    try:
                        if ov.winfo_exists():
                        ...
                        pass
                try:
                    if auth_dialog.winfo_exists():
                        auth_dialog.withdraw()
                except Exception:
                    pass

            def _show_lock_windows() -> None:
                if not getattr(self, "_lock_active", False):
                    return
                if not self.master.winfo_exists():
                    return
                ...
                try:
                    m_state = self.master.state()
                except Exception:
                    m_state = "normal"

                if m_state in ("iconic", "withdrawn"):
                    _hide_lock_windows()
                    return
                ...
                if ov is not None:
                    try:
                        if ov.winfo_exists() and ov.state() == "withdrawn":
                            ov.deiconify()
                    except Exception:
                        pass

                try:
                    if auth_dialog.winfo_exists() and auth_dialog.state() == "withdrawn":
                        auth_dialog.deiconify()
                except Exception:
                    pass
            ...
            def _on_master_unmap(_e=None) -> None:
                _hide_lock_windows()

            def _on_master_map(_e=None) -> None:
                self.master.after(0, _show_lock_windows)

            def _on_master_configure(_e=None) -> None:
                self._request_blur_update()
                self.master.after(0, _show_lock_windows)

            def _on_dialog_unmap(_e=None) -> None:
                try:
                    st = auth_dialog.state()
                except Exception:
                    return
                if st == "iconic":
                    try:
            ...
            bid_unmap = self.master.bind("<Unmap>", _on_master_unmap, add=True)
            bid_map = self.master.bind("<Map>", _on_master_map, add=True)
            bid_cfg = self.master.bind("<Configure>", _on_master_configure, add=True)

            auth_dialog.bind("<Unmap>", _on_dialog_unmap, add=True)
            ...
            try:
                if "bid_unmap" in locals():
                    self.master.unbind("<Unmap>", bid_unmap)
            except Exception:
                pass
            try:
                if "bid_map" in locals():
                    self.master.unbind("<Map>", bid_map)
            except Exception:
                pass
            try:
                if "bid_cfg" in locals():
                    self.master.unbind("<Configure>", bid_cfg)
            except Exception:
                pass
    ...
    def _create_menu(self):
        ...
        file_menu.add_command(label="Выход", command=self._tray_exit)
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--config", default="", help="Путь к config.json")
    p.add_argument("--salt", default="", help="Путь к salt.bin")
    p.add_argument("--pwfile", default="", help="Путь к passwords.json")
    return p.parse_args(argv)

def main(argv: list[str] | None = None) -> int:
    ns = _parse_args(sys.argv[1:] if argv is None else argv)
    _ensure_paths(ns.config, ns.salt, ns.pwfile)
