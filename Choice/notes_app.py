...
def _qt_message_handler(mode, context, message):
    try:
        if isinstance(message, str) and "QFileSystemWatcher::addPaths: list is empty" in message:
            return

        ctx = ""
        try:
            ctx = f"{getattr(context, 'file', '')}:{getattr(context, 'line', '')} {getattr(context, 'function', '')}"
        except Exception:
            ctx = ""
        msg = f"Qt[{int(mode)}] {ctx} | {message}"
        if QtMsgType is not None and mode == QtMsgType.QtFatalMsg:
...
def launcher_root() -> Path:
    env = (os.environ.get("LAUNCHER_ROOT") or "").strip()
    if env:
        return Path(env).resolve()
    cwd = Path.cwd().resolve()
    if (cwd / "launcher_config.json").exists() or (cwd / "Choice.exe").exists():
        return cwd
    here = Path(__file__).resolve()
    for p in [here.parent] + list(here.parents):
        if (p / "launcher_config.json").exists() or (p / "Choice.exe").exists():
            return p
    return here.parent

APPDIR = launcher_root()

DATA_DIR = APPDIR / "Data"
NOTES_DIR = APPDIR / "Notes"
PASSWORDS_DIR = APPDIR / "Passwords"

NOTES_ORDER_PATH = DATA_DIR / "notes_order.json"

STANDBY_CLEANER_PATH = DATA_DIR / "rammap.exe"
SALT_PATH = DATA_DIR / "salt.bin"
SETTINGS_PATH = DATA_DIR / "settings.ini"
ICON_PATH = DATA_DIR / "icon.ico"
TRAY_ICON_PATH = DATA_DIR / "tray_icon.ico"
FILE_ICON_PATH = DATA_DIR / "file.ico"
USER_DICT_PATH = DATA_DIR / "user_dictionary.txt"
NOTES_WIDGET_PATH = DATA_DIR / "desktop_notes.json"
NOTES_WIDGET_BAK = DATA_DIR / "desktop_notes.json.bak"

LOG_PATH = DATA_DIR / "crash.log"

PROCESS_PRIORITY_LEVELS = ("low", "below_normal", "normal", "above_normal", "high")

def qt_path(p: Path | str) -> str:
    return str(p)
...
def _setup_crash_logging(log_path: Path) -> None:
    logging.basicConfig(
        level=logging.INFO,
    ...
    try:
        faulthandler.enable(open(str(log_path), "a", encoding="utf-8"))
    except Exception:
        pass
...
RU_SUFFIXES_RU = tuple(sorted(_RU_SUFFIXES_SET, key=len, reverse=True))
...
class DesktopNotesWidget(QWidget):
    def __init__(self):
        ...
        self.notes_list.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
    ...
    def _load_pos(self):
        try:
            s = QSettings(qt_path(SETTINGS_PATH), QSettings.IniFormat)
    ...
    def _scan_app_notes(self) -> list[dict]:
        ...
        try:
            if not os.path.isdir(NOTES_DIR):
                return items
            max_depth = 3
            base = os.path.abspath(NOTES_DIR)
            ...
            for root, dirs, files in os.walk(base):
                ...
                if "note.json" in files:
                    note_json_paths.append(os.path.join(root, "note.json"))
            for file_path in note_json_paths:
                try:
                    with open(file_path, "r", encoding="utf-8") as f:
                        data = json.load(f)

                    title = (data.get("title") or data.get("text") or os.path.basename(os.path.dirname(file_path))).strip()
                    created = (data.get("timestamp") or data.get("created") or "").strip()
                    nid = (data.get("uuid") or data.get("id") or str(uuid.uuid4())).strip()
                    items.append({
                        "id": nid,
                        "text": title,
                        "created": created,
                        "updated": (data.get("updated") or created or "").strip(),
                        "source": "app",
                        "path": file_path,
                    })
                except Exception as e:
                    print("scan app notes error:", file_path, e)
            if not items:
                for folder in os.listdir(NOTES_DIR):
                    ...
                    if not os.path.isdir(folder_path):
                        continue
                    candidates = [fn for fn in os.listdir(folder_path) if fn.lower().endswith(".json")]
                    for fn in candidates:
                        ...
                        try:
                            ...
                            if not isinstance(data, dict):
                                continue
                            if not any(k in data for k in ("title", "text", "uuid", "id", "timestamp", "created")):
                                continue
                            title = (data.get("title") or data.get("text") or folder).strip()
                            created = (data.get("timestamp") or data.get("created") or "").strip()
                            nid = (data.get("uuid") or data.get("id") or str(uuid.uuid4())).strip()
                            items.append({
                                "id": nid,
    ...
    def _model_from_ui(self) -> list[dict]:
        ...
        for i in range(self.notes_list.count()):
            ...
            if src != "widget":
                continue
            txt = (it.text() or "").removeprefix("📌 ").strip()
            if not txt:
                continue
    ...
    def edit_note(self, item: QListWidgetItem):
        ...
        if src == "app":
            self.open_app_note_in_app(item)
            return
        cur_text = (item.text() or "").removeprefix("📌 ").strip()
        text, ok = QInputDialog.getMultiLineText(
    ...
    def open_app_note_in_app(self, item: QListWidgetItem) -> None:
        try:
            ...
            title_text = (item.text() or "").removeprefix("📝 ").strip()
...
class SpellCheckHighlighter(QSyntaxHighlighter):
    def __init__(self, parent):
        ...
        try:
            ...
            try:
                self.spell_checker_ru = _SpellChecker(language="ru")
            except Exception:
                self.spell_checker_ru = None
            try:
                self.spell_checker_en = _SpellChecker(language="en")
            except Exception:
                self.spell_checker_en = None
        except ImportError:
            self.spell_checker_ru = None
            self.spell_checker_en = None
        ...
        self.local_ignored: set[str] = set()
        self._load_user_dictionary()
        try:
            paths = []
            try:
                p = Path(USER_DICT_PATH) if not isinstance(USER_DICT_PATH, Path) else USER_DICT_PATH
                if p.exists():
                    paths.append(qt_path(p))
            except Exception:
                pass

            if paths:
                self._dict_watcher = QFileSystemWatcher(paths)
    ...
    def _load_user_dictionary(self) -> None:
        ...
        try:
            ...
            try:
                if self.spell_checker_ru and words:
                    self.spell_checker_ru.word_frequency.load_words(words)
            except Exception:
                pass
            try:
                if self.spell_checker_en and words:
                    self.spell_checker_en.word_frequency.load_words(words)
            except Exception:
                pass
        except Exception:
            pass
    ...
    def _is_known_local(self, w: str) -> bool:
        if not w:
            return False
        lw = self.normalize_e(w.strip().lower())
        if lw in self.user_words or lw in self.local_ignored:
            return True
        if not self.spell_checker_ru and not self.spell_checker_en:
            return True
        if re.search(r"[А-Яа-яЁё]", lw):
            if not self.spell_checker_ru:
                return True
            return len(self.spell_checker_ru.unknown([lw])) == 0
        if re.search(r"[A-Za-z]", lw):
            if not self.spell_checker_en:
                return True
            return len(self.spell_checker_en.unknown([lw])) == 0
        return True
    ...
    def highlightBlock(self, text: str) -> None:
        ...
        for match in matches:
            ...
            if left == "@" or right == "@":
                continue
            if "@" in text[max(0, start - 2): min(len(text), end + 2)]:
                continue
            if (left == "." or right == ".") and len(raw) <= 6:
                continue
            if len(raw) < 3:
                continue
            ...
            w = self.normalize_e(raw.lower())
            if w in self.user_words or w in self.local_ignored:
                continue

            if re.search(r"[А-Яа-яЁё]", w):
                bases = self._ru_base_forms(w)
                if any(self._is_known_local(b) for b in bases):
                    continue
                self.setFormat(match.start(), match.end() - match.start(), self.err_fmt)
                continue
            if not self._is_known_local(w):
                self.setFormat(match.start(), match.end() - match.start(), self.err_fmt)
...
def _global_excepthook(t, v, tb):
    try:
        logging.exception("Uncaught exception", exc_info=(t, v, tb))
    except Exception:
        pass
...
def paste_from_clipboard(widget):
    ...
    try:
        ...
        if tk is not None and ttk is not None and isinstance(widget, (tk.Entry, ttk.Entry)):
...
class FlowLayout(QLayout):
    ...
    def heightForWidth(self, width: int) -> int:
        try:
            return int(self.doLayout(QRect(0, 0, int(width), 0), testOnly=True))
        except Exception:
            return 0
    ...
    def doLayout(self, rect, testOnly):
        ...
        for item in self.itemList:
            ...
            if wid.property("flow_break") is True:
                x = rect.x()
                y += lineHeight + spaceY
                lineHeight = 0
                continue
        ...
        return int(y + lineHeight - rect.y())
...
class CustomTextEdit(QTextEdit):
    ...
    def __init__(
        ...
        self.setFont(QFont(self.DEFAULT_FONT_FAMILY, self.DEFAULT_FONT_SIZE_PT))
        self.setCurrentFont(QFont(self.DEFAULT_FONT_FAMILY))
        self.setFontPointSize(self.DEFAULT_FONT_SIZE_PT)
        self.document().setDefaultFont(
            QFont(self.DEFAULT_FONT_FAMILY, self.DEFAULT_FONT_SIZE_PT)
        )
        self.setAcceptDrops(True)
        self._ensure_default_typing_format()

    def _default_char_format(self) -> QTextCharFormat:
        fmt = QTextCharFormat()
        fmt.setFont(QFont(self.DEFAULT_FONT_FAMILY, self.DEFAULT_FONT_SIZE_PT))
        fmt.setFontPointSize(self.DEFAULT_FONT_SIZE_PT)
        return fmt

    def _strip_font_styles(self, html: str) -> str:
        if not html:
            return ""
    ...
    def _wrap_html_default(self, html: str, *, strip_fonts: bool = False) -> str:
        if strip_fonts:
            html = self._strip_font_styles(html)
        try:
            cur_fmt = self.currentCharFormat()
            font = cur_fmt.font()
            family = font.family() if (font and font.family()) else self.DEFAULT_FONT_FAMILY
            size = int(font.pointSize()) if (font and font.pointSize() > 0) else self.DEFAULT_FONT_SIZE_PT
        except Exception:
            family = self.DEFAULT_FONT_FAMILY
            size = self.DEFAULT_FONT_SIZE_PT
        if re.search(
            r"<\s*(table|div|p|ul|ol|li|h[1-6]|pre|blockquote)\b",
            html,
            flags=re.IGNORECASE,
        ):
            return (
                f"<div style=\"font-family:'{family}'; "
                f"font-size:{size}pt;\">{html}</div>"
            )
        return (
            f"<span style=\"font-family:'{family}'; "
            f"font-size:{size}pt;\">{html}</span>"
        )

    def _ensure_default_typing_format(self) -> None:
        try:
            cursor = self.textCursor()
            if cursor.hasSelection():
                return

            cur = QTextCharFormat(self.currentCharFormat())
            ...
            try:
                ...
                family = ""
            need_family = not (family or "").strip()
            need_size = cur.fontPointSize() <= 0

            if not (need_family or need_size):
                return

            if need_family:
                font.setFamily(self.DEFAULT_FONT_FAMILY)
            if need_size:
                font.setPointSize(self.DEFAULT_FONT_SIZE_PT)

            cur.setFont(font)
            self.setCurrentCharFormat(cur)
        except Exception:
            pass

    def set_default_font_size(self, size_pt: int) -> None:
        try:
            sz = max(1, int(size_pt))
        except Exception:
            return
        try:
            cur_fmt = QTextCharFormat(self.currentCharFormat())
            f = cur_fmt.font()
            f.setPointSize(sz)
            cur_fmt.setFont(f)
            ...
            pass
        try:
            doc_font = QFont(self.DEFAULT_FONT_FAMILY, sz)
            self.document().setDefaultFont(doc_font)

            try:
                self.DEFAULT_FONT_SIZE_PT = sz
            except Exception:
                pass
        except Exception:
            pass
    ...
    def insertHtml(self, html: str) -> None:
        wrapped = self._wrap_html_default(html, strip_fonts=True)
        super().insertHtml(wrapped)
    ...
    def insertFromMimeData(self, source: QMimeData) -> None:
        ...
        if source.hasHtml():
            ...
            return
        prev_fmt = QTextCharFormat(self.currentCharFormat())
        try:
            forced = QTextCharFormat(prev_fmt)
            font = forced.font()
            font.setFamily(self.DEFAULT_FONT_FAMILY)
            font.setPointSize(self.DEFAULT_FONT_SIZE_PT)
            forced.setFont(font)
            self.setCurrentCharFormat(forced)

            super().insertFromMimeData(source)
        finally:
            try:
                self.setCurrentCharFormat(prev_fmt)
            except Exception:
                pass
    ...
    def keyPressEvent(self, event):
        if event.key() in (Qt.Key_Return, Qt.Key_Enter):
            ...
            normalized = block_text.lstrip("\u202a\u202c\ufeff\u200b\xa0 ").replace("\xa0", " ")
            ...
            if m:
                ...
                if after_marker.strip() == "":
                    ...
                    super().keyPressEvent(event)
                    cursor = self.textCursor()
                    cursor.insertText("☐ ", self._default_char_format())
                    self.setTextCursor(cursor)
                    return
            b = normalized
            is_bullet = b.startswith("•") and (len(b) == 1 or b[1] == " ")
            if is_bullet:
                if b.strip() == "•":
                    cursor.beginEditBlock()
                    cursor.movePosition(QTextCursor.StartOfBlock)
                    rm_len = 2 if (len(b) >= 2 and b[1] == " ") else 1
                    cursor.movePosition(QTextCursor.Right, QTextCursor.KeepAnchor, rm_len)
                ...
                self.setTextCursor(cursor)
                return
            num_match = re.match(r"^(\d+)\.\s*", block_text.strip())
            if num_match:
                number = int(num_match.group(1))
                if re.fullmatch(rf"{number}\.\s*", block_text.strip()):
                    cursor.beginEditBlock()
                    cursor.movePosition(QTextCursor.StartOfBlock)
                    cursor.movePosition(
                        QTextCursor.Right,
                    ...
                    super().keyPressEvent(event)
                    cursor = self.textCursor()
                    cursor.insertText(f"{number + 1}. ", self._default_char_format())
                    self.setTextCursor(cursor)
                    return
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if event.button() == Qt.LeftButton:
            ...
            if word == "☐":
                word_cursor.insertText("☑", self._default_char_format())
                return
            elif word == "☑":
                word_cursor.insertText("☐", self._default_char_format())
                return
    ...
    def replace_word(self, cursor: QTextCursor, new_word: str) -> None:
        cursor.beginEditBlock()
        cursor.removeSelectedText()
        cursor.insertText(new_word, self._default_char_format())
        cursor.endEditBlock()
...
class _CentralWidgetProxy:
    ...
    def setVisible(self, v: bool):
        self._w.setVisible(v)

    def isVisible(self) -> bool:
        return self._w.isVisible()
...
class NotesApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.notes_folder = NOTES_DIR
        self.text_edit: CustomTextEdit | None = None
        self.tags_label: QLabel | None = None
        self.attachments_panel: QWidget | None = None
        self.attachments_layout: QHBoxLayout | None = None
        self.attachments_scroll: QScrollArea | None = None
        ...
        self.settings = QSettings(qt_path(SETTINGS_PATH), QSettings.IniFormat)
        self._apply_process_priority_from_settings()
        self._timer_mode = self.settings.value("timer/mode", "countdown")
        self._timer_total = int(self.settings.value("timer/total", 25 * 60, type=int))
        ...
        QTimer.singleShot(450, self._init_clipboard_watch)
        self._background_loader = None
        QTimer.singleShot(800, self._start_background_preload)
        self.tray_icon = None
        self._tray_inited = False
        QTimer.singleShot(350, self._init_tray_icon)
    ...
    def _load_note_from_path(file_path: str) -> Note:
        try:
            ...
            raise
        try:
            note_uuid = data.get("uuid") or data.get("id") or None
            if note_uuid:
                cache_path = os.path.join(DATA_DIR, f"note_cache_{note_uuid}.json")
                if os.path.exists(cache_path):
                    try:
                        with open(cache_path, "r", encoding="utf-8") as cf:
                            cache_data = json.load(cf)
                        cached_mtime = cache_data.get("note_mtime")
                        try:
                        ...
                        if (
                            ...
                            note_dict = cache_data.get("note")
                            if isinstance(note_dict, dict):
                                note = Note.from_dict(note_dict)
                                if not isinstance(getattr(note, "content_txt", ""), str):
                                    note.content_txt = note_dict.get("content_txt", "")
                                return note
                    except Exception:
                        pass
        except Exception:
    ...
    def _strip_default_font_styles(self, html: str) -> str:
        if not html:
            return ""
        html = re.sub(
            r'font-family\s*:\s*(?:["\']?\s*Times New Roman\s*["\']?)\s*;?',
            "",
            html,
            flags=re.IGNORECASE,
        )
        html = re.sub(
            r'font-size\s*:\s*14(?:\.0+)?pt\s*;?',
            "",
            html,
            flags=re.IGNORECASE,
        )
        html = re.sub(
            r'font-size\s*:\s*14(?:\.0+)?px\s*;?',
            "",
            html,
            flags=re.IGNORECASE,
        )
        html = re.sub(r'\sstyle="\s*"', "", html, flags=re.IGNORECASE)
        html = re.sub(r"\sstyle='\s*'", "", html, flags=re.IGNORECASE)
        return html
    ...
    def _apply_preloaded_notes(self, notes: list[Note]) -> None:
        ...
        self._apply_saved_notes_order()
        self.refresh_notes_list()
        last_uuid = (
            self.settings.value("lastNoteUUID", "")
            or self.settings.value("lastNoteUuid", "")
            or self.settings.value("ui/lastOpenedNoteUuid", "")
        )
        opened = False
        if last_uuid:
            for i in range(self.notes_list.count()):
                ...
                if n and getattr(n, "uuid", None) == last_uuid:
                    ...
                    self.load_note(self.notes_list.currentItem())
                    opened = True
                    break
        if not opened:
            for i in range(self.notes_list.count()):
        ...
        if not hasattr(self, "reminder_timer") or self.reminder_timer is None:
            self.setup_reminder_timer()

    def _init_clipboard_watch(self) -> None:
        if getattr(self, "_clipboard_connected", False):
            return
        if not getattr(self, "_clipboard_watch_enabled", True):
            return
    ...
    def _install_dialog_probe(self):
    ...
    def _init_editor_defaults(self) -> None:
        qf = QFont("Times New Roman", 14)
        self.text_edit.document().setDefaultFont(qf)
        self.text_edit.setFont(qf)
        fmt = QTextCharFormat()
    ...
    def init_ui(self) -> None:
        ...
        self.setWindowIcon(QIcon(qt_path(ICON_PATH)))
        ...
        self.export_note_button = QPushButton("Экспорт")
        ...
        self.clear_search_button.setFixedWidth(90)
        self.clear_search_button.clicked.connect(lambda: self.search_field.setText(""))
        ...
        btns.addWidget(self.export_note_button)
        ...
        self._sc_new.activated.connect(self.create_new_note)
        self._sc_save = QShortcut(QKeySequence("Ctrl+S"), self)
        self._sc_save.activated.connect(lambda: self.save_note_quiet(force=True))
        self._sc_focus_search = QShortcut(QKeySequence("Ctrl+F"), self)
        self._sc_focus_search.activated.connect(lambda: self.search_field.setFocus())
        self._sc_export = QShortcut(QKeySequence("Ctrl+Shift+E"), self)
        self._sc_export.activated.connect(self.export_note)
        self._sc_import = QShortcut(QKeySequence("Ctrl+Shift+I"), self)
        self._sc_import.activated.connect(self.import_note)
        ...
        if not self.settings.contains("spellcheck/online_enabled"):
            self.settings.setValue("spellcheck/online_enabled", True)
        self.spell_online_enabled = self.settings.value(
            "spellcheck/online_enabled", True, type=bool
        )
        ...
        _cw_lay = QVBoxLayout(self.central_widget)
        _cw_lay.setContentsMargins(0, 0, 0, 0)
        _cw_lay.setSpacing(0)
        _cw_lay.addWidget(editor_combined, 1)
        self.setCentralWidget(self.central_widget)
        self.dock_editor = _CentralWidgetProxy(editor_combined)
        self.init_toolbar()
        self.left_panel_splitter = QSplitter(Qt.Vertical)

        def _mk_section(title: str, widget: QWidget) -> QWidget:
            wrap = QWidget()
            lay = QVBoxLayout(wrap)
            lay.setContentsMargins(6, 6, 6, 6)
            lay.setSpacing(6)
            lbl = QLabel(title)
            lbl.setObjectName("left_section_title")
            lay.addWidget(lbl, 0)
            lay.addWidget(widget, 1)
            return wrap

        self._left_section_notes = _mk_section("Заметки", self.notes_list)
        self._left_section_history = _mk_section("История", self.history_widget)
        self._left_section_buttons = _mk_section("Управление", buttons_widget)
        self.left_panel_splitter.addWidget(self._left_section_notes)
        self.left_panel_splitter.addWidget(self._left_section_history)
        self.left_panel_splitter.addWidget(self._left_section_buttons)
        self.left_panel_splitter.setChildrenCollapsible(False)
        self.dock_notes_list = QDockWidget("Панель", self)
        self.dock_notes_list.setObjectName("dock_notes_list")
        self.dock_notes_list.setWidget(self.left_panel_splitter)
        self.dock_notes_list.setAllowedAreas(Qt.LeftDockWidgetArea | Qt.RightDockWidgetArea)
        self.addDockWidget(Qt.LeftDockWidgetArea, self.dock_notes_list)
        self.dock_history = None
        self.dock_buttons = None
        ...
        self._add_field_panel.setVisible(False) 
        ...
        self.resizeDocks(
            [self.dock_toolbar], [120], Qt.Vertical
        )
        for dock in (
        ...
        self.export_note_button.clicked.connect(self.show_export_menu)
        ...
        self._dock_ratios = self._load_dock_ratios_from_settings()
    ...
    def _load_dock_ratios_from_settings(self) -> list[float]:
        ratios = self.settings.value("ui/dock_ratios") or [0.22, 0.56, 0.22]
        try:
            ratios = [float(r) for r in ratios]
        except Exception:
            ratios = [0.22, 0.56, 0.22]

        ratios = (ratios + [0.22, 0.56, 0.22])[:3]
        total = float(sum(ratios))
        if total <= 0:
            return [0.22, 0.56, 0.22]
    ...
    def _apply_dock_proportions(self):
        if not getattr(self, "dock_notes_list", None) or not getattr(self, "dock_editor", None):
            return
        if not getattr(self, "left_panel_splitter", None):
            return
        try:
            total_h = max(1, self.dock_notes_list.height())
            lst = self.notes_list
            count = lst.count()
            if count > 0:
            ...
            btn_h_min = 60
            btn_widget = getattr(self, "_left_section_buttons", None)
            btn_h_hint = btn_widget.sizeHint().height() if btn_widget else 120
            ...
            if hist_h < hist_h_min:
                ...
                hist_h = hist_h_min
            try:
                self.left_panel_splitter.setSizes([int(list_h_needed), int(hist_h), int(btn_h)])
            except Exception:
                pass
        ...
        try:
            ...
            if dlg.exec() == QDialog.Accepted:
                ...
                if text and hasattr(self, "text_edit"):
                    ...
                    cur.insertText(text, self.text_edit._default_char_format())
    ...
    def _current_dock_widths(self):
        l = max(1, self.dock_notes_list.width()) if getattr(self, "dock_notes_list", None) else 1
        c = max(1, self.dock_editor.width()) if getattr(self, "dock_editor", None) else 1
        r = max(1, self.dock_history.width()) if getattr(self, "dock_history", None) else 1
        return [l, c, r]
    ...
    def move_note_to_trash(self, note: Note) -> None:
        ...
        try:
            self._remove_uuid_from_notes_order(getattr(note, 'uuid', '') or '')
        except Exception:
            pass
    ...
    def save_note_to_specific_folder(self, note: Note, folder: str) -> None:
        ...
        try:
            if self.current_note and note.uuid == getattr(self, "current_note", None).uuid:
                doc = QTextDocument()
                doc.setHtml(self.text_edit.toHtml())
                note_dict["content_txt"] = doc.toPlainText()
            else:
                doc = QTextDocument()
                doc.setHtml(note.content or "")
                note_dict["content_txt"] = doc.toPlainText()
        except Exception:
            note_dict["content_txt"] = note_dict.get("content_txt", "")
        self._atomic_write_json(file_path, note_dict)
        try:
            self._write_note_cache(note, file_path=file_path, note_dict=note_dict)
        except Exception:
            pass

    def _note_cache_path(self, note_uuid: str) -> str:
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        safe = f"note_cache_{note_uuid}.json"
        return os.path.join(DATA_DIR, safe)

    def _atomic_write_cache(self, path: str, payload: dict) -> None:
        tmp = path + ".tmp"
        with open(tmp, "w", encoding="utf-8") as f:
    ...
    def _write_note_cache(self, note: Note, file_path: str | None = None, note_dict: dict | None = None) -> None:
        try:
            if note_dict is None:
                note_dict = note.to_dict()
            if "content_txt" not in note_dict or not isinstance(note_dict.get("content_txt"), str):
                note_dict["content_txt"] = strip_html(note_dict.get("content", "") or "")
            ...
            note_path = file_path or os.path.join(note_dir, "note.json")
            try:
                mtime = os.path.getmtime(note_path) if os.path.exists(note_path) else None
            except Exception:
                mtime = None

            cache_path = os.path.join(note_dir, "note_cache.json")
            payload = {"_note_mtime": mtime, "note": note_dict}
    ...
    def _rotate_backups(self, folder: str, keep: int = 10) -> None:
        try:
            files = []
            for fn in os.listdir(folder):
                if fn.startswith("note_backup_") and fn.endswith(".json"):
    ...
    def _load_note_json_with_recovery(self, note_folder: str) -> dict | None:
        note_path = os.path.join(note_folder, "note.json")
        cache_path = os.path.join(note_folder, "note_cache.json")
        try:
            note_mtime = os.path.getmtime(note_path)
        except Exception:
            note_mtime = None

        if note_mtime is not None and os.path.exists(cache_path):
            try:
                with open(cache_path, "r", encoding="utf-8") as f:
                    cached = json.load(f)
                if cached.get("_note_mtime") == note_mtime and isinstance(cached.get("note"), dict):
        ...
        try:
            data = _read_json(note_path)
            if note_mtime is not None:
                try:
                    self._atomic_write_json(cache_path, {"_note_mtime": note_mtime, "note": data})
                except Exception:
                    pass
            return data
        except Exception:
        ...
        backups = []
        try:
            for fn in os.listdir(note_folder):
                if fn.startswith("note_backup_") and fn.endswith(".json"):
                    fp = os.path.join(note_folder, fn)
                    try:
    ...
    def record_state_for_undo(self) -> None:
        ...
        if not note:
            if hasattr(self, "attachments_scroll") and self.attachments_scroll:
                self.attachments_scroll.setVisible(False)
            return
    ...
    def _init_tray_icon(self) -> None:
        if getattr(self, "_tray_inited", False):
            return
        try:
            if not QSystemTrayIcon.isSystemTrayAvailable():
                self._tray_inited = True
                return
        except Exception:
            self._tray_inited = True
            return
        try:
            self.tray_icon = QSystemTrayIcon(QIcon(qt_path(TRAY_ICON_PATH)), self)
    ...
    def load_notes_from_disk(self):
        ...
        for folder_name in sorted(os.listdir(self.notes_folder)):
            note_folder = os.path.join(self.notes_folder, folder_name)
            if not os.path.isdir(note_folder):
                continue
            data = self._load_note_json_with_recovery(note_folder)
            if not isinstance(data, dict):
                continue
            try:
                note = Note.from_dict(data)
    ...
    def save_note_to_file(self, note: Note) -> None:
        ...
        if os.path.exists(note_path):
            try:
                backup_name = f"note_backup_{int(time.time())}.json"
                backup_path = os.path.join(note_folder, backup_name)
                shutil.copy2(note_path, backup_path)
                self._rotate_backups(note_folder, keep=10)
            except Exception:
                pass

        try:
            payload = note.to_dict()
            self._atomic_write_json(note_path, payload)
            try:
                mtime = os.path.getmtime(note_path)
                cache_path = os.path.join(note_folder, "note_cache.json")
                self._atomic_write_json(cache_path, {"_note_mtime": mtime, "note": payload})
            except Exception:
                pass
        except Exception as e:
            raise e
        ...
        if ok and title:
            for note in self.notes:
                if note.title == title:
                    ...
                    return
            note_uuid = str(uuid.uuid4())
            ts_iso = QDateTime.currentDateTime().toString(Qt.ISODate)
            ts_human = QDateTime.fromString(ts_iso, Qt.ISODate).toString("dd.MM.yyyy HH:mm")
            note = Note(
                title=title,
                content="",
                tags=[],
            ...
            note.history_index = 0
            note_dir = os.path.join(
                NOTES_DIR, NotesApp.safe_folder_name(title, note_uuid, ts_iso)
            )
            os.makedirs(note_dir, exist_ok=True)
            self.notes.append(note)
    ...
    def _get_autosave_debounce_ms(self) -> int:
        try:
            ms = int(self.settings.value("autosave/debounce_ms", 350))
        except Exception:
            ms = 350
        if ms < 150:
            ms = 150
    ...
    def show_export_menu(self):
        if not getattr(self, "current_note", None):
            QMessageBox.information(self, "Экспорт", "Сначала выберите заметку для экспорта.")
            return

        menu = QMenu(self)
        act_pdf = menu.addAction("Экспорт в PDF")
        act_txt = menu.addAction("Экспорт в TXT")
        act_docx = menu.addAction("Экспорт в DOCX")
        act_json = menu.addAction("Экспорт заметки (JSON)")
        ...
        def _safe_call(fn_name: str):
            ...
            if callable(fn):
                ...
                QMessageBox.warning(self, "Экспорт", f"Функция {fn_name} не найдена в этой версии.")

        act_pdf.triggered.connect(lambda: _safe_call("export_current_note_pdf"))
        act_txt.triggered.connect(lambda: _safe_call("export_current_note_txt"))
        act_docx.triggered.connect(lambda: _safe_call("export_current_note_docx"))
        act_json.triggered.connect(lambda: _safe_call("export_note"))
        pos = self.export_note_button.mapToGlobal(QPoint(0, self.export_note_button.height()))
        menu.exec(pos)
    ...
    def restore_note_from_trash(self) -> None:
        ...
        self.load_notes_from_disk()
        try:
            restored_uuid = getattr(note, "uuid", "") or ""
            if restored_uuid:
                order = self._load_notes_order()
                pinned = list(order.get("pinned") or [])
                unpinned = list(order.get("unpinned") or [])
                if restored_uuid not in pinned and restored_uuid not in unpinned:
                    (pinned if getattr(note, "pinned", False) else unpinned).append(restored_uuid)
                    self._save_notes_order(pinned, unpinned)
        except Exception:
            pass
        self.refresh_notes_list()
        for i in range(self.notes_list.count()):
        ...
        try:
            if previous_note:
                try:
                    ...
                    html_content = self._persist_dropdown_values_in_html(html_content)
                except Exception:
                    html_content = previous_note.content
                previous_note.content = html_content
                try:
                    plain_text = self._html_to_plain_text(html_content)
                except Exception:
                    plain_text = ""
                import threading
                if not hasattr(self, "_note_save_locks"):
                    self._note_save_locks = {}
                lock = self._note_save_locks.get(previous_note.uuid)
                if lock is None:
                    lock = threading.Lock()
                    self._note_save_locks[previous_note.uuid] = lock
                note_snapshot = previous_note.to_dict()
                if not previous_note.reminder:
                    note_snapshot.pop("reminder", None)
                note_snapshot["content"] = html_content
                note_snapshot["content_txt"] = plain_text

                def _bg_save(snapshot: dict, title: str, uuid_: str, ts: str | QDateTime | None) -> None:
                    with lock:
                        note_dir = os.path.join(NOTES_DIR, NotesApp.safe_folder_name(title, uuid_, ts))
                        os.makedirs(note_dir, exist_ok=True)
                        file_path = os.path.join(note_dir, "note.json")
                        try:
                            if os.path.exists(file_path):
                                with open(file_path, "r", encoding="utf-8") as bf:
                                    prev_json = bf.read()
                                safe_title = (
                                    re.sub(r"[^a-zA-Zа-яА-Я0-9 _\-\.\(\)]", "", title)
                                    .strip()
                                    .replace(" ", "_")
                                    or "note"
                                )[:100]
                        ...
                        try:
                            ...
                            pass
                        try:
                            if hasattr(self, "_write_note_cache"):
                                self._write_note_cache(previous_note, file_path=file_path, note_dict=snapshot)
                        except Exception:
                            pass
                threading.Thread(
                    target=_bg_save,
                    args=(note_snapshot, previous_note.title, previous_note.uuid, previous_note.timestamp),
                    daemon=True,
                ).start()
            self.current_note = note
    ...
    def _ensure_typing_defaults(self) -> None:
        fmt = self.text_edit.currentCharFormat()
        f = fmt.font()
        family = (f.family() or "").strip()
        if not family:
        ...
        fmt.setFont(f)
        self.text_edit.setCurrentCharFormat(fmt)
        self.text_edit.document().setDefaultFont(QFont("Times New Roman", 14))
        if hasattr(self, "font_combo"):
            self.font_combo.setCurrentFont(QFont(fmt.font().family() or "Times New Roman"))
        if hasattr(self, "font_size_spin"):
            self.font_size_spin.setValue(int(fmt.fontPointSize() or 14))
    ...
    def _list_attachment_files(self, note: Note | None) -> list[tuple[str, str]]:
        ...
        for filename in os.listdir(note_dir):
            if (
                ...
                or filename.startswith("note_backup_")
    ...
    def show_note_with_attachments(self, note: "Note | None") -> None:
        self.current_note = note
        self._attachments_render_token = getattr(self, "_attachments_render_token", 0) + 1
        token = self._attachments_render_token
        if not note:
            if hasattr(self, "attachments_scroll") and self.attachments_scroll:
                self.attachments_scroll.setVisible(False)
        ...
        self.settings.setValue("ui/lastOpenedNote", getattr(self, "current_note_path", ""))
        ...
        if hasattr(self, "attachments_layout"):
            ...
            if not attachments:
                ...
                loading_lbl = QLabel("Вложения загружаются…")
                loading_lbl.setObjectName("_attachments_loading_label")
                self.attachments_layout.addWidget(loading_lbl)
                note_dir_path = NOTES_DIR / NotesApp.safe_folder_name(note.title, note.uuid, note.timestamp)
                note_dir = qt_path(note_dir_path)
                try:
                    if self.attachments_watcher is None:
                    ...
                    if note_dir and note_dir_path.is_dir() and note_dir not in self.attachments_watcher.directories():
                ...
                def _render_batch() -> None:
                    if getattr(self, "_attachments_render_token", None) != token:
                        return
                    if not getattr(self, "current_note", None) or self.current_note.uuid != note.uuid:
                        return
                    if not hasattr(self, "attachments_layout") or not self.attachments_layout:
                        return
                    if state["i"] == 0:
                        for j in reversed(range(self.attachments_layout.count())):
                            w = self.attachments_layout.itemAt(j).widget()
                            if isinstance(w, QLabel) and w.objectName() == "_attachments_loading_label":
                                w.setParent(None)
                                w.deleteLater()
                    batch = 5
                    added = 0
                    while state["i"] < len(attachments) and added < batch:
                        filename, file_path = attachments[state["i"]]
                        state["i"] += 1
                        added += 1
                        item_widget = QWidget()
                        ...
                        if filename.lower().endswith(tuple(IMAGE_EXTENSIONS)):
                            ...
                            if not pixmap.isNull():
                                ...
                                icon_label.setPixmap(self.style().standardIcon(QStyle.SP_FileIcon).pixmap(32, 32))
                            layout.addWidget(icon_label)
                        else:
                            icon_label = QLabel()
                            if FILE_ICON_PATH.exists():
                                icon_label.setPixmap(
                                    QPixmap(qt_path(FILE_ICON_PATH)).scaled(48, 48, Qt.KeepAspectRatio, Qt.SmoothTransformation)
                                )
                            else:
                                icon_label.setPixmap(self.style().standardIcon(QStyle.SP_FileIcon).pixmap(32, 32))
                            layout.addWidget(icon_label)
                        ...
                        del_btn.clicked.connect(lambda _, path=file_path: self.delete_attachment_from_panel(path))
                        layout.addWidget(del_btn)
                        item_widget.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
                        if self.attachments_layout.count() > 0:
                            sep_lbl = QLabel(" - ")
                            sep_lbl.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
                            self.attachments_layout.addWidget(sep_lbl)
                        self.attachments_layout.addWidget(item_widget)
                    if state["i"] < len(attachments):
                        QTimer.singleShot(0, _render_batch)
                QTimer.singleShot(0, _render_batch)
        if hasattr(self, "tags_label"):
            self.tags_label.setText(f"Теги: {', '.join(note.tags) if note.tags else 'нет'}")

    def _apply_default_doc_font_fallback(self) -> None:
        f = QFont("Times New Roman", 14)
        doc = self.text_edit.document()
        doc.setDefaultFont(f)
        self.text_edit.setFont(f)
        self.text_edit.setCurrentFont(f)
        self.text_edit.setFontPointSize(14)
    ...
    def _refresh_attachments(self, *args) -> None:
        ...
        if args:
            ...
            if (
                ...
                or basename.startswith("note_cache_")
                or basename.startswith("note_backup_")
                or (basename.startswith("backup(") and basename.endswith(".json"))
    ...
    def _update_editor_visibility(self):
        ...
        self._add_field_panel.setVisible(has)
        if not has:
    ...
    def _force_default_note_font(self) -> None:
        f = QFont("Times New Roman", 14)
        self.text_edit.setCurrentFont(f)
        self.text_edit.setFontPointSize(14)
    ...
    def change_font_size(self, size: int) -> None:
        try:
            ...
            elif getattr(self, "_last_selection", None):
                start, end = self._last_selection
                doc_cursor = QTextCursor(self.text_edit.document())
                doc_cursor.setPosition(start)
                doc_cursor.setPosition(end, QTextCursor.KeepAnchor)
                doc_cursor.mergeCharFormat(fmt)
            else:
                cur_fmt = self.text_edit.currentCharFormat()
                cur_fmt.merge(fmt)
                self.text_edit.setCurrentCharFormat(cur_fmt)
                try:
                    if hasattr(self, "text_edit") and getattr(self.text_edit, "set_default_font_size", None):
                        self.text_edit.set_default_font_size(int(size))
                except Exception:
                    pass
            self.update_font_controls()
            self._set_unsaved(True)
            if getattr(self, "autosave_enabled", False):
                if hasattr(self, "save_note_quiet"):
                    self.save_note_quiet(force=True)
        except Exception:
            pass
    ...
    def _replace_selection_with_text(self, cursor: QTextCursor, new_text: str) -> None:
        ...
        cursor.insertText(new_text, self.text_edit._default_char_format())
    ...
    def _register_global_hotkeys(self) -> None:
        if get_keyboard_module is None:
            return
        self._unregister_global_hotkeys()
        try:
            hk_layout = get_keyboard_module.add_hotkey(
                "ctrl+shift+f12",
                lambda: self._invoke_in_main_thread(self.translate_layout_only),
                suppress=False,
            )
            hk_case = get_keyboard_module.add_hotkey(
                "ctrl+alt+f12",
    ...
    def _unregister_global_hotkeys(self) -> None:
        ...
        for hk in getattr(self, "_global_hotkeys", []):
            try:
                get_keyboard_module.remove_hotkey(hk)
            except Exception:
                pass
        self._global_hotkeys = []
    ...
    def insert_list(self, style: QTextListFormat.Style) -> None:
        ...
        if cursor.hasSelection():
            ...
            if style == QTextListFormat.ListDisc:
                cursor.insertText("• ", self.text_edit._default_char_format())
            elif style == QTextListFormat.ListDecimal:
                cursor.insertText("1. ", self.text_edit._default_char_format())
        ...
        if cursor.hasSelection():
            ...
            while True:
                ...
                if text.strip() and not (
                    ...
                    block_cursor.insertText("☐ ", self.text_edit._default_char_format())
                if block == end_block:
                    break
                block = block.next()
        else:
    ...
    def apply_sorting(self) -> None:
        ...
        if index == 0:
            self.notes.sort(key=lambda note: (note.title or "").lower(), reverse=reverse)
            self.refresh_notes_list()
        elif index == 1:
            self.notes.sort(key=lambda note: (note.timestamp or ""), reverse=reverse)
            self.refresh_notes_list()
        elif index == 2:
            self.notes.sort(
                key=lambda note: (not bool(note.favorite), (note.title or "").lower()),
                reverse=reverse,
            )
            self.refresh_notes_list()
    ...
    def refresh_notes_list(self) -> None:
        ...
        def _add_note(note):
            title = note.title
            timestamp = QDateTime.fromString(note.timestamp, Qt.ISODate)
            date_str = timestamp.toString("dd.MM.yyyy HH:mm")
        ...
        if pinned_notes and unpinned_notes:
            ...
            line.setStyleSheet("""
            QFrame {
                background-color: #000000;      /* тело линии */
                border-top: 1px solid #ffffff;  /* обводка сверху */
                border-bottom: 1px solid #ffffff;/* обводка снизу */
                border-left: none;
                border-right: none;
            }
            """)
            sep_item.setSizeHint(QSize(0, line.height()))
    ...
    def _adjust_notes_list_height(self) -> None:
        def _set_left_section_h(list_h: int) -> None:
            try:
                wrap = getattr(self, "_left_section_notes", None)
                if wrap is None:
                    return
                lay = wrap.layout()
                if lay is None:
                ...
                spacing = int(lay.spacing() if lay.spacing() is not None else 0)
                title_h = 0
                try:
                    it0 = lay.itemAt(0)
                    w0 = it0.widget() if it0 is not None else None
                    if w0 is not None:
                        title_h = int(w0.sizeHint().height())
                except Exception:
                    title_h = 0
                total = int(m.top() + m.bottom() + title_h + spacing + int(list_h))
                wrap.setMinimumHeight(total)
                wrap.setMaximumHeight(total)
            except Exception:
                pass
        ...
        if self.notes_list.count() == 0:
            ...
            _set_left_section_h(60)
            return
        rows = self.notes_list.count()
        spacing = 0
        try:
            spacing = int(getattr(self.notes_list, "spacing", lambda: 0)())
        except Exception:
            spacing = 0
        total_rows_h = 0
        fm_h = self.notes_list.fontMetrics().height() + 6
        for i in range(rows):
            h = 0
            try:
                h = int(self.notes_list.sizeHintForRow(i))
            except Exception:
                h = 0
            if h <= 0:
                h = fm_h
            total_rows_h += max(1, h)
        frame = self.notes_list.frameWidth() * 2
        margins = self.notes_list.contentsMargins()
        extra = frame + margins.top() + margins.bottom()
        total_h = total_rows_h + max(0, rows - 1) * max(0, spacing) + extra
        total_h += 4
        self.notes_list.setMinimumHeight(total_h)
        self.notes_list.setMaximumHeight(total_h)
        _set_left_section_h(total_h)
    ...
    def add_tag_to_note(self) -> None:
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if added:
                ...
                self.show_toast("Нет новых тегов", "Все введённые теги уже есть у заметки.")
    ...
    def _rebuild_attachments_panel(
        ...
        for filename, file_path in attachments:
            ...
            if filename.lower().endswith(tuple(IMAGE_EXTENSIONS)):
                ...
                if FILE_ICON_PATH.exists():
                    icon_label.setPixmap(
                        QPixmap(qt_path(FILE_ICON_PATH)).scaled(
        ...
        for r in range(rows):
            ...
            for c in range(cols):
                if r == 0 and c == 0 and selected_html is not None:
                    ...
                    cell_content = "&nbsp;"
                cells_html.append(
                    f"<td style=\'min-width:1em; padding:3px; border:1px solid {border_color}; font-family:Times New Roman; font-size:14pt;\'>{cell_content}</td>"
                )
            rows_html.append("<tr>" + "".join(cells_html) + "</tr>")
        table_html = (
            f"<table border='1' cellspacing='0' cellpadding='3' "
            f"style=\'border-collapse:collapse; border:1px solid {border_color}; font-family:Times New Roman; font-size:14pt;\'>"
            + "".join(rows_html)
            + "</table>"
        )
        cursor.beginEditBlock()
    ...
    def _timer_context_menu(self, pos):
        ...
        act_mode = menu.addAction(
            "Режим: " + ("Обратный отсчёт" if self._timer_mode == "countdown" else "Секундомер")
        )
        sender = self.sender()
        try:
            global_pos = (
    ...
    def show_reminder_dialog(self, note):
        ...
        when = QDateTime.fromString(note_time, Qt.ISODate)
        when_text = when.toString('dd.MM.yyyy HH:mm') if when.isValid() else note_time
        msg.setText(f"Напоминание по заметке:\n\n{note.title}\n\n{when_text}")

        open_btn = msg.addButton("Открыть заметку", QMessageBox.AcceptRole)
        snooze5_btn = msg.addButton("Отложить 5 мин", QMessageBox.ActionRole)
        snooze15_btn = msg.addButton("Отложить 15 мин", QMessageBox.ActionRole)
        snooze60_btn = msg.addButton("Отложить 1 час", QMessageBox.ActionRole)
        ok_btn = msg.addButton("ОК", QMessageBox.RejectRole)

        msg.setWindowFlag(Qt.WindowStaysOnTopHint, True)
        msg.exec()
        clicked = msg.clickedButton()

        def _snooze(mins: int):
            dt = QDateTime.currentDateTime().addSecs(mins * 60)
            note.reminder = dt.toString(Qt.ISODate)
        ...
        elif clicked == snooze5_btn:
            _snooze(5)
        elif clicked == snooze15_btn:
            _snooze(15)
        elif clicked == snooze60_btn:
            _snooze(60)
        else:
            note.reminder_shown = True
    def show_note_context_menu(self, position):
        ...
        if note_in_trash:
            ...
            delete_action.triggered.connect(lambda: self.delete_note_completely(note))
            menu.addAction(delete_action)
        else:
    ...
    def add_menu_bar(self):
        ...
        perf_menu = tools_menu.addMenu("Приоритет приложения")
        act_pr_low = QAction("Низкий", self)
        act_pr_low.triggered.connect(lambda: self.set_process_priority_ui("low"))
        perf_menu.addAction(act_pr_low)
        act_pr_bn = QAction("Ниже среднего", self)
        act_pr_bn.triggered.connect(lambda: self.set_process_priority_ui("below_normal"))
        perf_menu.addAction(act_pr_bn)
        act_pr_norm = QAction("Обычный", self)
        act_pr_norm.triggered.connect(lambda: self.set_process_priority_ui("normal"))
        perf_menu.addAction(act_pr_norm)
        act_pr_an = QAction("Выше среднего", self)
        act_pr_an.triggered.connect(lambda: self.set_process_priority_ui("above_normal"))
        perf_menu.addAction(act_pr_an)
        act_pr_high = QAction("Высокий", self)
        act_pr_high.triggered.connect(lambda: self.set_process_priority_ui("high"))
        perf_menu.addAction(act_pr_high)
        ...
        system_menu = self.menuBar().addMenu("Система")
        act = QAction("Показать текущий приоритет", self)
        act.triggered.connect(self.show_process_priority)
        system_menu.addAction(act)
        prio_sub = system_menu.addMenu("Установить приоритет")
        for lvl, title in [
            ("low", "Низкий"),
            ...
            a = prio_sub.addAction(title)
            a.triggered.connect(lambda checked=False, lvl=lvl: self.set_process_priority_ui(lvl))
        ...
        trash_menu.addAction(act)
        act = QAction("Удалить навсегда", self)
    ...
    def show_settings_window(self):
        ...
        online_spell_cb = QCheckBox(dialog)
        ...
        autosave_debounce = QSpinBox(dialog)
        autosave_debounce.setRange(150, 5000)
        autosave_debounce.setSingleStep(50)
        try:
            autosave_debounce.setValue(
                int(self.settings.value("autosave/debounce_ms", 350))
            )
        except Exception:
            autosave_debounce.setValue(350)
        autosave_debounce.setToolTip(
            "Задержка автосохранения после последнего ввода.\n"
            "Меньше — чаще запись на диск; больше — меньше нагрузки."
        )
        layout.addRow("Автосохранение (мс):", autosave_debounce)
        buttons = QDialogButtonBox(
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            try:
                self.settings.setValue(
                    "autosave/debounce_ms", int(autosave_debounce.value())
                )
            except Exception:
                pass
            self.init_theme()
    ...
    def _update_timer_ui(self) -> None:
        if not hasattr(self, "timer_btn") or self.timer_btn is None:
            return
    ...
    def export_note(self):
        if not self.current_note:
            QMessageBox.warning(self, "Экспорт", "Сначала выберите заметку для экспорта.")
            return

        items = ["JSON (полный)", "TXT", "PDF", "DOCX", "Markdown (TXT)", "ZIP (папка заметки)"]
        choice, ok = QInputDialog.getItem(self, "Экспорт", "Формат:", items, 0, False)
        if not ok:
            return

        note = self.current_note
        if choice.startswith("JSON"):
            file_path, _ = QFileDialog.getSaveFileName(self, "Экспорт заметки", f"{note.title}.json", "JSON Files (*.json)")
            if not file_path:
                return
            try:
                with open(file_path, "w", encoding="utf-8") as f:
        ...
        if choice == "TXT":
            self.export_current_note_txt()
            return
        if choice == "PDF":
            self.export_current_note_pdf()
            return
        if choice == "DOCX":
            self.export_current_note_docx()
            return

        if choice.startswith("Markdown"):
            file_path, _ = QFileDialog.getSaveFileName(self, "Экспорт заметки", f"{note.title}.md", "Markdown (*.md)")
            if not file_path:
                return
            try:
                text = note.content_txt or strip_html(note.content)
                header = f"# {note.title}\n\n"
                tags = ""
                try:
                    _tags = getattr(note, 'tags', None) or []
                    if _tags:
                        tags = "\n\n---\nТеги: " + ", ".join(_tags) + "\n"
                except Exception:
                    tags = ""
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(header + text + tags)
                self.show_toast("Экспорт завершён")
            except Exception as e:
                QMessageBox.critical(self, "Ошибка", f"Ошибка экспорта: {e}")
            return
        ...
        if choice.startswith("ZIP"):
            ...
            if not os.path.isdir(note_folder):
                QMessageBox.critical(self, "Ошибка", "Папка заметки не найдена.")
                return
            file_path, _ = QFileDialog.getSaveFileName(self, "Экспорт заметки", f"{note.title}.zip", "ZIP (*.zip)")
            if not file_path:
                return
            try:
                import zipfile
                with zipfile.ZipFile(file_path, "w", compression=zipfile.ZIP_DEFLATED) as z:
                    for root, _, files in os.walk(note_folder):
                        for fn in files:
                            ...
                            rel = os.path.relpath(fp, os.path.dirname(note_folder))
                            z.write(fp, rel)
                self.show_toast("Экспорт завершён")
            except Exception as e:
                QMessageBox.critical(self, "Ошибка", f"Ошибка экспорта: {e}")
    ...
    def import_note(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Импорт заметки", "", "Заметки (*.json *.txt *.md)")
        if not file_path:
            return
        try:
            ...
            if ext == '.json':
                ...
                note_data["title"] = (note_data.get("title", "") + " (импорт)").strip()
                imported_note = Note.from_dict(note_data)
                imported_note.content_txt = note_data.get('content_txt', '')
            else:
                with open(file_path, "r", encoding="utf-8") as f:
                    text = f.read()
                base = os.path.basename(file_path)
                title = os.path.splitext(base)[0]
                imported_note = Note(title=title + " (импорт)", content=text)
                imported_note.content_txt = text
    ...
    def handle_combined_search(self):
        ...
        def _matches(note):
            matches_tag_filter = tag in [t.lower() for t in note.tags] if tag else True
            if not text:
                return matches_tag_filter
            note_tags_joined = " ".join([t for t in (note.tags or [])]).lower()
            matches_search_by_tags = text in note_tags_joined if note_tags_joined else False
            if mode == "Заголовок":
                matches_search_main = text in (note.title or "").lower()
            elif mode == "Напоминание":
                matches_search_main = bool(note.reminder) and text in str(note.reminder).lower()
            else:
                content_plain = getattr(note, "content_txt", "") or ""
                if not content_plain:
                    ...
                    content_plain = doc.toPlainText()
                    note.content_txt = content_plain
                matches_search_main = text in content_plain.lower()
            return matches_tag_filter and (matches_search_main or matches_search_by_tags)
    ...
    def list_attachments_for_current_note(self):
        if not self.current_note:
            return
        if hasattr(self, "_rebuild_attachments_panel"):
            self._rebuild_attachments_panel(self.current_note)
    ...
    def _resize_editor_dock(self, delta_px: int) -> None:
        ...
        total_w = max(1, self.width())
        min_editor = 300
        max_left = max(80, total_w - min_editor)
        new_left = max(80, min(max_left, left_w - int(delta_px)))
        try:
            self.resizeDocks([left_dock], [new_left], Qt.Horizontal)
        except Exception:
            try:
                left_dock.setMinimumWidth(80)
                left_dock.resize(new_left, left_dock.height())
            except Exception:
                pass
    ...
    def _load_notes_order(self) -> dict:
        path = str(NOTES_ORDER_PATH)
        try:
            if os.path.isfile(path):
                with open(path, "r", encoding="utf-8") as f:
                    data = json.load(f) or {}
                pinned = data.get("pinned") or []
                unpinned = data.get("unpinned") or []
                if isinstance(pinned, list) and isinstance(unpinned, list):
                    return {
                        "pinned": [str(x) for x in pinned],
                        "unpinned": [str(x) for x in unpinned],
                    }
        except Exception:
            pass
        return {"pinned": [], "unpinned": []}

    def _save_notes_order(self, pinned: list[str], unpinned: list[str]) -> None:
        try:
            os.makedirs(str(DATA_DIR), exist_ok=True)
            payload = {"pinned": list(pinned), "unpinned": list(unpinned)}
            with open(str(NOTES_ORDER_PATH), "w", encoding="utf-8") as f:
                json.dump(payload, f, ensure_ascii=False, indent=2)
        except Exception:
            pass

    def _remove_uuid_from_notes_order(self, note_uuid: str) -> None:
        if not note_uuid:
            return
        order = self._load_notes_order()
        pinned = [u for u in (order.get("pinned") or []) if u != note_uuid]
        unpinned = [u for u in (order.get("unpinned") or []) if u != note_uuid]
        self._save_notes_order(pinned, unpinned)

    def _timestamp_sort_key(self, note: "Note") -> int:
        try:
            dt = QDateTime.fromString(getattr(note, "timestamp", "") or "", Qt.ISODate)
            if dt.isValid():
                return int(dt.toSecsSinceEpoch())
        except Exception:
            pass
        try:
    ...
    def _apply_saved_notes_order(self) -> None:
        if not isinstance(getattr(self, "notes", None), list) or not self.notes:
            return
        order = self._load_notes_order()
        pinned_order = order.get("pinned") or []
        unpinned_order = order.get("unpinned") or []
        by_uuid: dict[str, Note] = {}
        for n in self.notes:
            if getattr(n, "uuid", None):
                by_uuid[str(n.uuid)] = n
        pinned_notes = [n for n in by_uuid.values() if getattr(n, "pinned", False)]
        unpinned_notes = [n for n in by_uuid.values() if not getattr(n, "pinned", False)]
        pinned_sorted_rest = sorted(
            [n for n in pinned_notes if str(n.uuid) not in pinned_order],
            key=self._timestamp_sort_key,
            reverse=True,
        )
        unpinned_sorted_rest = sorted(
            [n for n in unpinned_notes if str(n.uuid) not in unpinned_order],
            key=self._timestamp_sort_key,
            reverse=True,
        )
        pinned_final: list[Note] = [
            by_uuid[u]
            for u in pinned_order
            if u in by_uuid and getattr(by_uuid[u], "pinned", False)
        ]
        unpinned_final: list[Note] = [
            by_uuid[u]
            for u in unpinned_order
            if u in by_uuid and (not getattr(by_uuid[u], "pinned", False))
        ]
        pinned_final.extend(pinned_sorted_rest)
        unpinned_final.extend(unpinned_sorted_rest)
        self.notes = pinned_final + unpinned_final

    def handle_note_reorder(self) -> None:
        try:
            pinned: list[str] = []
            unpinned: list[str] = []
            seen: set[str] = set()
            for i in range(self.notes_list.count()):
                ...
                if item is None:
                    continue
                note = item.data(Qt.UserRole)
                if not note or not getattr(note, "uuid", None):
                    continue
                uid = str(note.uuid)
                if uid in seen:
                    continue
                ...
                if getattr(note, "pinned", False):
                    ...
                    unpinned.append(uid)
            self._save_notes_order(pinned, unpinned)
            current_uuid = getattr(getattr(self, "current_note", None), "uuid", None)
            self._apply_saved_notes_order()
            self.refresh_notes_list()
            if current_uuid:
                for i in range(self.notes_list.count()):
                    it = self.notes_list.item(i)
                    n = it.data(Qt.UserRole)
                    if n and getattr(n, "uuid", None) == current_uuid:
                        self.notes_list.setCurrentRow(i)
                        break
        except Exception:
            try:
    ...
    def _on_text_selection_changed(self) -> None:
        try:
            cur = self.text_edit.textCursor()
            if cur.hasSelection():
                self._last_selection = (int(cur.selectionStart()), int(cur.selectionEnd()))
        except Exception:
            pass
    ...
    def safe_folder_name(title, note_uuid, timestamp=None):
        ...
        elif isinstance(timestamp, QDateTime):
            dt = timestamp
        else:
            dt = QDateTime.currentDateTime()
        dt_str = dt.toString("dd.MM.yyyy HH-mm")
        uuid_short = (
            note_uuid if note_uuid.startswith("UUID_") else f"UUID({note_uuid[:8]})"
        )
        return f"{safe_title} от {dt_str}, {uuid_short}"
            ...
            try:
                ...
                try:
                    with open(log_path, "a", encoding="utf-8") as f:
                        ...
                        f.write(
                            f"Ошибка при загрузке плагина {getattr(module, '__name__', '<unknown>')}: {e}\n"
                        )
        ...
        for fname in os.listdir(plugins_folder):
            ...
            try:
                ...
                if spec is None or spec.loader is None:
                    raise RuntimeError(f"spec/loader is None for plugin: {fname}")
                module = importlib.util.module_from_spec(spec)
                ...
                try:
                    with open(log_path, "a", encoding="utf-8") as f:
                        ...
                        f.write(f"Ошибка при загрузке плагина {plugin_name} ({fname}): {e}\n")
                        traceback.print_exc(file=f)
                except Exception:
                    pass
                continue
    ...
    def save_plugins_state(self):
        plugins_path = os.path.join(DATA_DIR, "plugins_state.json")
        try:
            self._atomic_write_json(plugins_path, self.plugins_state)
        except Exception:
            try:
    ...
    def _ensure_autosave_timer(self) -> None:
        if hasattr(self, "_autosave_timer") and isinstance(getattr(self, "_autosave_timer"), QTimer):
            return
        self._autosave_timer = QTimer(self)
        self._autosave_timer.setSingleShot(True)
        self._autosave_timer.timeout.connect(self._perform_autosave)

        self._autosave_interval_ms = 0

        if not hasattr(self, "_autosave_in_progress"):
    ...
    def on_text_changed_autosave(self):
        ...
        self._ensure_autosave_timer()
        try:
            ms = self._get_autosave_debounce_ms()
            self._autosave_timer.start(ms)
        except Exception:
            logging.exception("Failed to start autosave timer")
    ...
    def _perform_autosave(self) -> None:
        ...
        try:
            if hasattr(self, "autosave_current_note"):
    ...
    def _set_process_priority(self, level: str) -> bool:
        level = (level or "").strip().lower()
        if level not in _WIN_PRIORITY_CLASS:
            level = "normal"
        ...
        try:
            ...
            ok = SetPriorityClass(hproc, _WIN_PRIORITY_CLASS[level])
            return bool(ok)
        except Exception:
            return False

    def _get_process_priority(self) -> str:
        if platform.system().lower() != "windows":
            return "normal"
        try:
            kernel32 = ctypes.windll.kernel32
            ...
            GetCurrentProcess.restype = ctypes.c_void_p
            GetPriorityClass.argtypes = [ctypes.c_void_p]
            GetPriorityClass.restype = ctypes.c_uint32
            hproc = GetCurrentProcess()
            cls = int(GetPriorityClass(hproc))

            for k, v in _WIN_PRIORITY_CLASS.items():
                if int(v) == cls:
                    return k
        except Exception:
            pass
    ...
    def show_process_priority(self) -> None:
        try:
            level = self._get_process_priority()
            self.show_toast(f"Текущий приоритет процесса: {level}")
        except Exception:
            pass

    def _apply_process_priority_from_settings(self) -> None:
        level = "normal"
        try:
            if hasattr(self, "settings") and self.settings is not None:
                level = self.settings.value("system/process_priority", "normal")
        except Exception:
            level = "normal"
        self._set_process_priority(level)

    def set_process_priority_ui(self, level: str) -> None:
        level = (level or "").strip().lower()
        if level not in PROCESS_PRIORITY_LEVELS:
            level = "normal"

        logging.info(f"set_process_priority_ui: requested={level}")

        ok = self._set_process_priority(level)

        logging.info(f"set_process_priority_ui: applied={ok}, level={level}")

        try:
            if hasattr(self, "settings") and self.settings is not None:
                self.settings.setValue("system/process_priority", level)
        except Exception:
            logging.exception("Failed to save process priority to settings")

        try:
            if ok:
                self.show_toast(f"Приоритет процесса: {level}")
            else:
                self.show_toast("Не удалось применить приоритет процесса (или ОС не Windows).")
        except Exception:
            logging.exception("Failed to show toast after priority change")
