...
class DesktopNotesWidget(QWidget):
    ...
    def resizeEvent(self, e):
        super().resizeEvent(e)
        if hasattr(self, "_grip_top_right"):
            s = self._grip_top_right.sizeHint()
            m = 4
            self._grip_top_right.move(self.width() - s.width() - m, m)
    ...
    def _apply_auto_min_height(self) -> None:
        """Применяет вычисленную минимальную высоту (и, при необходимости, поднимает текущее окно)."""
...
class LauncherWindow(QMainWindow):
    ...
    def launch_tg_outgoing(self):
        self._start_external("Отчет: исходящие в Telegram", ["TelegramOutgoingReporter.exe"])
