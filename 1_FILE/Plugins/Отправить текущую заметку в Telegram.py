...
def _exe_base_dir() -> str:
    try:
        ...
        pass
    return _app_dir_from_plugin()

def _dev_launcher_path() -> str:
    return os.path.join(_exe_base_dir(), "Dev", "launcher.py")
...
class _Config:
    def __init__(self):
        ...
        self.send_dev_launcher: bool = False
        self.send_as_html: bool = True
    ...
    def load(self):
        ...
        self.send_dev_launcher = bool(data.get("send_dev_launcher", False))
        self.send_as_html = bool(data.get("send_as_html", True))
    ...
    def save(self):
        data = {
            ...
            "send_dev_launcher": bool(self.send_dev_launcher),
            "send_as_html": bool(self.send_as_html),
        }
        tmp = self.path + ".tmp"
...
def _tg_send_message(token: str, chat_id: str, text: str, parse_mode: str | None = None) -> dict:
        payload = {"chat_id": chat_id, "text": text, "disable_web_page_preview": True}
        if parse_mode:
            payload["parse_mode"] = parse_mode
        return _http_post_form(_tg_api_url(token, "sendMessage"), payload)
...
class _SendDialog(QDialog):
    def __init__(self, parent_window):
        ...
        self.cb_html = QCheckBox("Сохранять форматирование (HTML)")
        self.cb_html.setChecked(self.cfg.send_as_html)
        layout.addRow("", self.cb_html)
        ...
        self.cb_dev = QCheckBox(r"Отправлять Dev\launcher.py (рядом с EXE)")
        dev_path = _dev_launcher_path()
        dev_exists = os.path.isfile(dev_path)
        self.cb_dev.setChecked(self.cfg.send_dev_launcher if dev_exists else False)
        self.cb_dev.setEnabled(dev_exists)
        self.cb_dev.setToolTip("" if dev_exists else f"Файл не найден: {dev_path}")
        layout.addRow("", self.cb_dev)
        btn_row = QHBoxLayout()
    ...
    def get_values(self) -> tuple[str, str, bool, bool, bool]:
        token = self.ed_token.text().strip()
        chat_id = self.ed_chat.text().strip()
        send_atts = self.cb_attach.isChecked()
        send_dev = self.cb_dev.isChecked()
        dev_path = _dev_launcher_path()
        dev_exists = os.path.isfile(dev_path)
        send_dev = self.cb_dev.isChecked() and dev_exists
        if not dev_exists:
            self.cb_dev.setChecked(False)
        send_html = self.cb_html.isChecked()
        self.cfg.token, self.cfg.chat_id = token, chat_id
        self.cfg.send_attachments = send_atts
        self.cfg.send_dev_launcher = send_dev 
        self.cfg.send_as_html = send_html
        self.cfg.save()
        return token, chat_id, send_atts, send_dev, send_html
        ...
        if resp.get("ok"):
            try:
                if hasattr(self.parent(), "show_toast"):
                    self.parent().show_toast("Тест отправлен в Telegram", ms=1500)
            except Exception:
                pass
            return
        else:
...
def _send_text_chunked_with_numbers(token: str, chat_id: str, text: str, limit: int = 3500) -> None:
    parts = _split_message(text, limit)
    total = len(parts)
    for i, part in enumerate(parts, 1):
        prefix = f"({i}/{total})\n" if total > 1 else ""
        resp = _tg_send_message(token, chat_id, prefix + part)
        if not resp.get("ok", False):
            raise RuntimeError(resp.get("description", "Telegram error"))
...
def _clean_html_for_telegram(html: str) -> str:
    if not html:
        return ""
    html = html.replace("\r\n", "\n").replace("\r", "\n")
    html = re.sub(r"(?is)<\s*(html|head|body)\b[^>]*>", "", html)
    html = re.sub(r"(?is)</\s*(html|head|body)\s*>", "", html)
    html = re.sub(r'<input[^>]+type=["\']?checkbox["\']?[^>]+checked[^>]*>', "☑ ", html, flags=re.I)
    html = re.sub(r'<input[^>]+type=["\']?checkbox["\']?[^>]*>', "☐ ", html, flags=re.I)
    for tag in ("h1","h2","h3","h4","h5","h6"):
        html = re.sub(fr"(?is)<{tag}\b[^>]*>(.*?)</{tag}>", r"<b>\1</b><br>", html)
    html = re.sub(r"(?is)<li\b[^>]*>(.*?)</li>", r"• \1<br>", html)
    html = re.sub(r"(?is)</?(ul|ol)\b[^>]*>", "", html)
    html = re.sub(r"(?is)</p\s*>", r"<br>", html)
    html = re.sub(r"(?is)<p\b[^>]*>", "", html)
    html = re.sub(r"(?is)</div\s*>", r"<br>", html)
    html = re.sub(r"(?is)<div\b[^>]*>", "", html)
    html = re.sub(r"(?is)</?(span|font)\b[^>]*>", "", html)
    html = re.sub(r"(?is)<(img|table|thead|tbody|tfoot|tr|td|th|iframe|video|audio)\b[^>]*>.*?</\1\s*>", "", html)
    html = re.sub(r"(?is)<(img|br|hr)\b[^>]*/?>", lambda m: "<br>" if m.group(1).lower()=="br" else "", html)

    def _strip_attrs(m: re.Match) -> str:
        tag = m.group(1).lower()
        attrs = m.group(2) or ""
        if tag == "a":
            href = re.search(r'href\s*=\s*(".*?"|\'.*?\'|[^\s>]+)', attrs, flags=re.I)
            return f"<a href={href.group(1)}>" if href else "<a>"
        if tag in ("b","strong","i","em","u","s","del","ins","code","pre","br"):
            return f"<{tag}>"
        return f"<{tag}>"

    html = re.sub(r"<\s*([a-zA-Z0-9]+)\b([^>]*)>", _strip_attrs, html)
    html = re.sub(r"</\s*strong\s*>", "</b>", html)
    html = re.sub(r"</\s*em\s*>", "</i>", html)
    html = html.replace("&nbsp;", " ")
    html = re.sub(r"(?is)(<br>\s*){3,}", "<br><br>", html).strip()
    return html

def _build_html_for_note(note) -> str:
    title = note.title or "(без названия)"
    try:
    ...
    if getattr(note, "tags", None):
        try:
            t = ", ".join(note.tags)
            if t:
                tags_html = f"<br><i>Теги: {t}</i>"
        except Exception:
            pass

    body_raw = getattr(note, "content", "") or ""
    body_html = _clean_html_for_telegram(body_raw)

    header = f"<b>📝 {title}</b>"
    if date_str:
        header += f"<br><i>Дата: {date_str}</i>"
    return f"{header}{tags_html}<br><br>{body_html}".strip()
...
def _send_current_note_impl(parent):
    ...
    try:
        ...
        if dlg.exec() != QDialog.Accepted:
            return
        token, chat_id, send_attachments, send_dev, send_as_html = dlg.get_values()
        ok_t, msg_t = _validate_token(token)
        ok_c, msg_c = _validate_chat_id(chat_id)
        ...
        sent_text_ok = False
        html_text  = _build_html_for_note(note)
        plain_text = _build_text_for_note(note)
        try:
            if send_as_html and len(html_text) <= 3500:
                resp = _tg_send_message(token, chat_id, html_text, parse_mode="HTML")
                if not resp.get("ok", False):
                    raise RuntimeError(resp.get("description", "Telegram error"))
                sent_text_ok = True
            else:
                _send_text_chunked_with_numbers(token, chat_id, plain_text, limit=3500)
                sent_text_ok = True
                try:
                    if send_as_html:
                        tmp_path = os.path.join(_data_dir(), "note_export.html")
                        with open(tmp_path, "w", encoding="utf-8") as f:
                            f.write(html_text)
                        _send_file_via_bot(token, chat_id, tmp_path, caption="Заметка (HTML)")
                except Exception:
                    pass

        except Exception as e:
            QMessageBox.critical(parent, "Telegram", f"Не удалось отправить сообщение:\n{e}")
            return
        ...
        if send_dev:
            ...
            if os.path.isfile(dev_path):
                try:
                    _send_file_via_bot(token, chat_id, dev_path, caption="Dev/launcher.py")
                except Exception as e:
                    QMessageBox.warning(parent, "Telegram", f"Текст отправлен, но Dev/launcher.py не отправился:\n{e}")
            else:
                QMessageBox.information(parent, "Telegram", f"Файл не найден:\n{dev_path}")

        try:
            if hasattr(parent, "show_toast"):
                parent.show_toast("Отправлено в Telegram", ms=1500)
            elif hasattr(parent, "tray_icon"):
                parent.tray_icon.showMessage("Telegram", "Заметка отправлена.", QSystemTrayIcon.Information, 1500)
        except Exception:
            pass
    finally:
        _sending = False
...
def on_enable(parent):
    global _action_send, _installed
    if _installed:
        return
