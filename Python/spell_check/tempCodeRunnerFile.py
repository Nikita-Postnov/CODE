...
class Plugin:
    ...
    def __init__(self, app):
        ...
        Args:
            app: Ссылка на основное приложение, к которому подключается плагин
        """
        self.app = app
    ...
    def register(self):
        """Метод для регистрации плагина в приложении"""
        self.app.register_plugin(self)
        self.enabled = True
        return self

    def unregister(self):
        """Метод для отключения плагина от приложения"""
        self.app.unregister_plugin(self)
        self.enabled = False
        return self
    ...
    def get_config(self):
        """Получить текущую конфигурацию плагина"""
        return self.config
...
class SpellCheckerApp:
    ...
    def remove_word_and_update(self, word):
        if self.dictionary.remove(word):
            self.update_result(f"Слово '{word}' удалено", "green")
            self.check_spelling()  # Добавьте полную проверку
            self.highlight_errors()
            self.check_punctuation()
        else:
            self.update_result(f"Ошибка удаления '{word}'", "red")
